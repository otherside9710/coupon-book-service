  ...
  subgraph CI
    ...
    flowchart LR
      ...
      subgraph CICD_PIPELINE
        GitHub["Repo (GitHub)"] --> CI_SERVER["CI (GitHub Actions / CodeBuild)"]
        CI_SERVER --> ECR["ECR / Registry"]
        ECR --> ALB
      end
    ...
    sequenceDiagram
      ...
      alt locked and owner != requestor
        ...
        API->>Postgres: SELECT COUNT(*) FROM coupon_redemptions WHERE user_id = ? AND coupon_book_id = ?
        API->>Postgres: BEGIN TRANSACTION
        API->>Postgres: SELECT * FROM coupon_codes WHERE id = ? FOR UPDATE
        API->>Postgres: INSERT INTO coupon_redemptions (coupon_code_id, coupon_book_id, user_id, redeemed_at)
        API->>Postgres: UPDATE coupon_codes SET status = 'REDEEMED', redeemed_by_user_id = ?, redeemed_at = now() WHERE id = ?
        API->>Postgres: COMMIT
